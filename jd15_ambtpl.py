...
try:
    os.makedirs("amb/jd15")

    print('The directories have been made.')
    
    input('Insert your ckd files in amb/jd15 and then run the tool again to deserialize the tapes.')

except:
    pass

for tpl in os.listdir("amb/jd15"):
    ...
    "COMPONENTS": [{
        ...
        for amb in range(ambcount):
            ...
            amb_zlib=m.read(4)
            m.read(4)
            ambtpl["COMPONENTS"][0]["soundList"][0]["files"].append(amb_path.replace('jd2015','maps')+amb_file)
